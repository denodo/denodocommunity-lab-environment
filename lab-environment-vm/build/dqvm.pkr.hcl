...
build {
  ...
  provisioner "shell" {
    inline = [
		...
		"cd ${var.home-path}",
		"git clone ${local.common-lab-url}",
		"chown -R ${local.user-denodo}:${local.user-denodo} ${local.common-lab-prj-name}",
		...
		"echo '    ${var.home-path}/${local.common-lab-prj-name}' >> /etc/motd", 
		"echo '' >> /etc/motd",
		"echo -n 'Visit ' >> /etc/motd",
		"echo -en '\\033[4;36m' >> /etc/motd",
		"echo -n '${local.common-lab-readme}' >> /etc/motd",
		"echo -en '\\033[0m' >> /etc/motd",
		"echo ' for accessing the ${local.common-lab-prj-name} documentation.' >> /etc/motd",
		"echo -n 'Visit ' >> /etc/motd",
		"echo -en '\\033[4;36m' >> /etc/motd",
